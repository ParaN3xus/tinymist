...
class NightlyUtils {
    ...
    async generateChangelog(newVersion, currentCommit, typstRev, typstBaseCommit) {
        const currentDate = new Date().toISOString().split('T')[0];

        const changelogEntry = `## v${newVersion} - [${currentDate}]

Nightly Release at [commit ${currentCommit.slice(0, 7)}](https://github.com/Myriad-Dreamin/tinymist/commit/${currentCommit}), using [ParaN3xus/typst rev ${typstRev.slice(0, 7)}](https://github.com/ParaN3xus/typst/commit/${typstRev}), a.k.a. [typst/typst commit ${typstBaseCommit}](https://github.com/typst/typst/commit/${typstBaseCommit}).
...
`;
        ...
        try {
            ...
            for (let i = 0; i < lines.length; i++) {
                if (lines[i].startsWith('## v')) {
                    if (firstReleaseIndex === -1) {
                        ...
                        const match = lines[i].match(/## (v[0-9][^\s]*)/);
                        if (match) {
            ...
            let finalEntry = changelogEntry;
            if (previousVersion) {
                finalEntry = finalEntry.replace('{{PREV_VERSION}}', previousVersion);
            } else {
                finalEntry = finalEntry.replace('{{PREV_VERSION}}', 'HEAD~1');
            }

            if (firstReleaseIndex >= 0) {
                const newLines = [
                    ...lines.slice(0, firstReleaseIndex),
                    finalEntry,
                    ...lines.slice(firstReleaseIndex)
                ];
                await fs.writeFile(changelogPath, newLines.join('\n'));
            } else {
                // append
            ...
            console.warn(`Warning: Could not update CHANGELOG.md: ${e.message}`);
            // create
    ...
    calculateNewVersion(currentVersion, releaseType) {
        if (releaseType === 'canary') {
            ...
            const patchVersion = parseInt(versionParts[2]) + 2;
    ...
    try {
        switch (command) {
            ...
            case 'generate-changelog': {
                const newVersion = process.argv[4];
                const currentCommit = process.argv[5];
                const typstRev = process.argv[6];
                const typstBaseCommit = process.argv[7];
                if (!newVersion || !currentCommit || !typstRev || !typstBaseCommit) {
                    throw new Error('Usage: generate-changelog <version> <commit> <typst-rev> <typst-base-commit>');
                }
                await utils.generateChangelog(newVersion, currentCommit, typstRev, typstBaseCommit);
            ...
            default:
                ...
                console.error('  generate-changelog <version> <commit> <typst-rev> <typst-base-commit>');
