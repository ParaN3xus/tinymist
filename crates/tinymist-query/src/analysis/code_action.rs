...
impl<'a> CodeActionWorker<'a> {
    ...
    pub fn autofix_file_not_found(
        ...
        let root_path = self
            .ctx
            .path_for_id(PathOrStr::Str("/".into()).resolve(file_id).ok()?.intern())
            .ok()?;
        let path_in_workspace = file_id.vpath().join(importing.as_str()).ok()?;
        let new_path = path_in_workspace.realize(root_path.as_path());
        let new_file_url = path_to_url(&new_path).ok()?;

        let edit = self.create_file(new_file_url, false);

        let file_to_create = unix_slash(Path::new(path_in_workspace.get_with_slash()));
        let action = CodeAction {
